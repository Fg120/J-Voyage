<!-- ========== HEADER ========== -->
<header
    class="flex flex-wrap md:justify-start md:flex-nowrap bg-neutral-800 backdrop-blur-md border-b border-neutral-800 sticky top-0 z-50 transition-all duration-300">
    <nav
            ...
            <div
                ...
                <div class="py-2 md:py-0 flex flex-col md:flex-row md:items-center gap-0.5 md:gap-1">
                    ...
                    <div class="flex flex-wrap items-center gap-x-1.5">
                        @auth
                            ...
                            <div class="hs-dropdown [--placement:bottom-right] relative inline-flex">
                                <button id="hs-dropdown-account" type="button"
                                    class="inline-flex items-center gap-x-2 px-2 py-1 rounded-lg hover:bg-neutral-700 transition-colors focus:outline-hidden text-white"
                                ...
                                <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-60 bg-neutral-800 shadow-black/50 shadow-xl rounded-lg mt-2 after:h-4 after:absolute after:-bottom-4 after:start-0 after:w-full before:h-4 before:absolute before:-top-4 before:start-0 before:w-full"
                                    ...
                                    <div class="p-1.5 space-y-0.5">
                                        ...
                                        <form method="POST" action="{{ route('logout') }}" class="w-full">
                                            @csrf
                                            <button type="submit"
                                                class="w-full flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-red-400 hover:bg-neutral-700 focus:outline-hidden">
                                                Logout
                                            </button>
                                        </form>
                                    </div>
                        ...
                        @else
                            <a href="{{ route('login') }}"
                                class="py-2 px-2.5 inline-flex items-center font-medium text-sm rounded-lg bg-indigo-300 text-black hover:bg-indigo-400">
                                LOGIN
                            </a>
                        @endauth
                    </div>
