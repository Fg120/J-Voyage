    ...
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md">
            ...
            <div>
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold uppercase">{{ str_replace('_', ' ', $transaksi->metode_pembayaran) }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            @if($transaksi->bukti_pembayaran)
                <div class="border rounded-lg p-2 inline-block">
                    @if(Str::endsWith($transaksi->bukti_pembayaran, '.pdf'))
                        <a href="{{ Storage::url($transaksi->bukti_pembayaran) }}" target="_blank" class="flex items-center gap-2 text-purple-600 hover:underline">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                            Lihat Dokumen PDF
                        </a>
                    @else
                        <img src="{{ Storage::url($transaksi->bukti_pembayaran) }}" alt="Bukti Pembayaran" class="max-w-md rounded shadow-sm">
                    @endif
                </div>
            @else
        ...
        <div class="mt-8 flex gap-4 border-t pt-6">
            <form action="{{ route('pengelola.transaksi.approve', $transaksi->id) }}" method="POST">
                @csrf
                <button type="submit" class="px-5 py-3 font-medium leading-5 text-white transition-colors duration-150 bg-green-600 border border-transparent rounded-lg active:bg-green-600 hover:bg-green-700 focus:outline-none focus:shadow-outline-green">
                    Setujui Transaksi
                </button>
            </form>

            <form action="{{ route('pengelola.transaksi.reject', $transaksi->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menolak transaksi ini?');">
                @csrf
                <button type="submit" class="px-5 py-3 font-medium leading-5 text-white transition-colors duration-150 bg-red-600 border border-transparent rounded-lg active:bg-red-600 hover:bg-red-700 focus:outline-none focus:shadow-outline-red">
                    Tolak Transaksi
                </button>
            </form>
        </div>
